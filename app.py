...
st.set_page_config(page_title="Gestión ITSE Nacional", layout="wide")

@st.cache_data
def load_data():
    # 1. CARGA DE BASES (Asegura estos nombres en tu PC)
    df_l = pd.read_csv("CONSOLIDADO_FINAL_LIMA.csv", encoding='latin-1')
    df_p = pd.read_csv("CONSOLIDADO_FINAL_PROVINCIAS.csv", encoding='latin-1')
    ...
    def aplicar_reglas(row):
        # Campos comunes
        prob_est = str(row.get('PROBLEMAS ESTRUCTURALES', '')).upper().strip()
        itse_status = str(row.get('ITSE', '')).upper().strip()
        
        # REGLA GENERAL: SIN ITSE
        if prob_est == 'SI' or itse_status == 'SIN ITSE':
            return "SIN ITSE"
        
        # REGLAS ESPECÍFICAS SEGÚN ZONA
        if row['ZONA'] == 'LIMA':
            ...
            # Si no hay FVEN, revisa FECHA LIMITE
...
# Buscador y Filtros
busqueda = st.text_input("🔍 Buscar por local, dirección o código:", "")
...
def style_alerta(val):
    color = 'white'
    if "VENCIDO" in val or "SIN ITSE" in val: color = '#FFCCCC' # Rojo suave
    elif "VIGENTE" in val: color = '#CCFFCC' # Verde suave
